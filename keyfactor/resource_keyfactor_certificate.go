...
func (r resourceKeyfactorCertificate) enrollPFXV2(ctx context.Context, plan *KeyfactorCertificate) (
...
func (r resourceKeyfactorCertificate) parseSans(ctx context.Context, plan *KeyfactorCertificate) (
	[]string, []string,
...
func (r resourceKeyfactorCertificate) parseMetadata(
	ctx context.Context,
	plan *KeyfactorCertificate,
...
func (r resourceKeyfactorCertificate) enrollCSR(
...
func (r resourceKeyfactorCertificate) Read(
	...
	cResp, crtGetErr := r.p.client.GetCertificateContext(args)
	//if crtGetErr != nil {
	//	tflog.Error(ctx, "Error calling GetCertificateContext")
	//	response.Diagnostics.AddWarning(
	//		ERR_SUMMARY_CERTIFICATE_RESOURCE_READ,
	//		fmt.Sprintf("Could not retrieve certificate '%s' from Keyfactor Command: "+crtGetErr.Error(), state.ID.Value),
	//	)
	...
	//leafPEM, chain, pKey, dErr := downloadCertificate(certificateIdInt, r.p.client, state.KeyPassword.Value, csr != "")
	...
	var (
		...
		chainPEM   = ""
		pKeyPEM    = ""
		recoverErr diag.Diagnostics
		downloadErr diag.Diagnostics
		ecodeErr   error
		haveCert   bool
		haveChain  bool
		haveKey    bool
	...
	if crtGetErr != nil || (cResp != nil && cResp.HasPrivateKey) {
		tflog.Info(
			ctx, fmt.Sprintf(
				"Attempting to recover certificate %s with private key from Keyfactor Command",
				state.ID.Value,
			),
		)
		pKeyPEM, leafPEM, chainPEM, recoverErr = recoverPrivateKeyFromKeyfactorCommand(
			ctx,
			collectionIdInt,
			collectionIdInt,
			lookupPassword,
		...
		if pKeyPEM != "" {
			...
			tflog.Debug(
				ctx,
				fmt.Sprintf(
					"Successfully recovered private key for certificate '%s'(%d) from Keyfactor Command.",
					state.ID.Value,
					certificateIdInt,
				),
			)
		...
		if leafPEM != "" {
			...
			tflog.Debug(
				ctx,
				fmt.Sprintf(
					"Successfully recovered certificate '%s'(%d) from Keyfactor Command.",
					state.ID.Value,
				),
			)
		}
		...
		if chainPEM != "" {
			...
			tflog.Debug(
				ctx,
				fmt.Sprintf(
					"Successfully recovered chain for certificate '%s'(%d) from Keyfactor Command.",
					state.ID.Value,
					certificateIdInt,
				),
			)
	...
	if leafPEM == "" && cResp != nil {
		...
		var lErr error
		leafPEM, lErr = encodeCertificate(ctx, cResp.ContentBytes, certificateIdInt)
		if lErr != nil {
			tflog.Error(ctx, "Error decoding certificate response from Keyfactor Command.")
			response.Diagnostics.AddError(
				ERR_SUMMARY_CERTIFICATE_RESOURCE_READ,
				fmt.Sprintf(
					"Error decoding certificate response from Keyfactor Command for certificate '%s': "+lErr.Error(),
					state.ID.Value,
				),
			)
			return
		...
		if leafPEM == "" || chainPEM == "" {
			...
			leafPEM, chainPEM, downloadErr = downloadCertificateFromKeyfactorCommand(
				ctx,
				certificateIdInt,
				collectionIdInt,
				r.p.client,
		...
		*/
		...
		if state.CSR.Value != "" {
			...
			result = KeyfactorCertificate{
				...
				CommonName:         types.String{Value: leaf.Subject.CommonName, Null: true},
				Locality:           types.String{Value: strings.Join(leaf.Subject.Locality, ","), Null: isNullString(strings.Join(leaf.Subject.Locality, ","))},
				State:              types.String{Value: strings.Join(leaf.Subject.Province, ","), Null: isNullString(strings.Join(leaf.Subject.Province, ","))},
				Country:            types.String{Value: strings.Join(leaf.Subject.Country, ","), Null: isNullString(strings.Join(leaf.Subject.Country, ","))},
				Organization:       types.String{Value: strings.Join(leaf.Subject.Organization, ","), Null: isNullString(strings.Join(leaf.Subject.Organization, ","))},
				OrganizationalUnit: types.String{Value: strings.Join(leaf.Subject.OrganizationalUnit, ","), Null: isNullString(strings.Join(leaf.Subject.OrganizationalUnit, ","))},
	...
	tfsdk.ImportResourceStateRequest,
		...
		if certificateData.HasPrivateKey {
			...
			tflog.Info(
				ctx,
				fmt.Sprintf(
					"Downloading certificate '%s'(%d) without private key from Keyfactor Command.",
					certificateData.IssuedCN,
					certificateData.Id,
				),
			)
			tflog.Debug(ctx, "Calling downloadCertificate")
			leafPEM, chainPEM, apiDiags = downloadCertificateFromKeyfactorCommand(
				ctx,
				certificateData.Id,
				0,
				r.p.client,
